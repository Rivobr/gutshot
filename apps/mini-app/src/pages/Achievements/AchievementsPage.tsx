...
export function AchievementsPage(): JSX.Element {
  ...
  const views = useMemo(
    ...
    [catalog, achievementTexts, profile?.achievementProgress, profile?.unlockedAchievements],
  );

  const sections = useMemo(
    () =>
      groupAchievements(views).map((section) => ({
        ...section,
        items: sortAchievementsByAvailability(section.items),
      })),
    [views],
  );

  if (isLoading || catalogLoading || !profile) {
    return <Loader />;
  }

  const unlockedCount = views.filter((item) => item.unlocked).length;
  const totalXpEarned = views
    .filter((item) => item.unlocked)
    .reduce((sum, item) => sum + item.xp, 0);

  const selected = selectedId ? (views.find((item) => item.id === selectedId) ?? null) : null;
    ...
    >
      ...
      <div className="relative px-4 pt-5 pb-3">
        ...
        >
          ...
          >
            {unlockedCount} / {views.length} · заработано {totalXpEarned.toLocaleString('ru-RU')} XP
          </p>
        </motion.div>
      </div>

      {sections.map((section, sectionIndex) => (
        <section key={section.group} className="relative px-3 mt-4">
          <div className="flex items-center gap-2 px-1 mb-2.5">
            <span
              className="serif font-semibold uppercase"
              style={{ fontSize: 13, color: '#C89A3D', letterSpacing: '0.12em' }}
            >
              {section.label}
            </span>
            <span className="sans" style={{ fontSize: 10, color: '#5C5346' }}>
              {section.items.filter((item) => item.unlocked).length}/{section.items.length}
            </span>
            <span
              aria-hidden
              className="flex-1"
              style={{ height: 1, background: 'rgba(199,154,61,0.18)' }}
            />
          </div>
...
function AchievementCard({
  ...
  return (
    <motion.button
      ...
      transition={{ duration: 0.38, delay: 0.03 + index * 0.03, ease: [0.22, 1, 0.36, 1] }}
      whileTap={{ scale: 0.97 }}
      ...
      >
        ...
        <div className="mt-2 w-full">
          <div className="flex items-center justify-between gap-2 mb-1.5">
            ...
            >
              XP {item.xp.toLocaleString('ru-RU')}
            </span>
            <span
              className="sans"
              style={{ fontSize: 10, fontWeight: 600, color: done ? rarity.accent : '#6B614E' }}
            >
              {done ? 'Получено' : `${item.progress}/${item.target}`}
            </span>
          </div>
          <div
            style={{
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <div className="mt-3 w-full">
            <div className="flex items-center justify-between gap-3 mb-1.5">
              ...
              <span
                className="sans"
                style={{ fontSize: 11, color: done ? rarity.accent : '#6B614E', fontWeight: 600 }}
              >
                {done ? 'Получено' : `${item.progress}/${item.target}`}
              </span>
            </div>
