 ...
 * Запуск в api-контейнере: node scripts/resync-achievements.cjs
 */
const {
  PrismaClient,
  PlayerEventType,
...
} = require('../dist/src/common/constants/achievements-catalog.js');
...
async function collectMetrics(userId, thresholds) {
  ...
  const eventCount = (type) => eventCounts.find((row) => row.type === type)?._count._all ?? 0;
...
async function main() {
  ...
  const byId = {};
  for (const g of granted) byId[g.id] = (byId[g.id] ?? 0) + 1;

  console.log(
    JSON.stringify(
      { players: users.length, grants: granted.length, byAchievement: byId },
      null,
      2,
    ),
  );
