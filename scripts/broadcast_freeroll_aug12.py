#!/usr/bin/env python3
"""Одноразовая рассылка: фриролл 12 августа."""
...
TEXT = (
    '🔥 <b>Уже в эту среду — 12 августа — ФРИРОЛ!</b> 🔥\n'
    '\n'
    'Мест осталось совсем немного ⏳\n'
    'Регистрация продолжается, но только пока есть свободные места.\n'
    '\n'
    'Успей занять своё место за столом и сыграть абсолютно бесплатно ♠️\n'
    '\n'
    '📅 <b>Среда, 12 августа</b>\n'
    '🎟 Вход — <b>0₽</b>\n'
    '⚠️ Количество мест ограничено\n'
    '\n'
    'Записывайся прямо сейчас, пока ещё есть возможность попасть в турнир 🔥'
)
...
def get_users():
    sql = (
        'SELECT "telegramId", COALESCE("firstName",\'\') FROM "User" '
...
def main():
    ...
    users = get_users()
    print(f'recipients: {len(users)} dry_run={dry}')
    if not users:
        print('no users')
        return
    if not validate(users[0][0]):
        print('ABORT: ticket did not validate, not sending')
        sys.exit(1)
    if dry:
        print('DRY RUN OK — not sending')
        return
    ...
    for tid, _name in users:
        ...
        body = {
            ...
            'reply_markup': {
                'inline_keyboard': [
                    [{'text': '♠️ Записаться в клуб', 'web_app': {'url': entry_url(ticket)}}]
                ]
            },
        }
        for attempt in range(3):
            try:
                ...
                if e.code == 429:
                    ...
                    continue
                failed += 1
                errors.append((tid, f'{e.code} {raw[:100]}'))
                break
            except Exception as e:
                if attempt == 2:
                    failed += 1
                    errors.append((tid, str(e)[:100]))
                else:
                    time.sleep(1)
        time.sleep(0.08)
