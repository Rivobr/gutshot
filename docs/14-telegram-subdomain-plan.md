...
1. Создайте **A**-записи:
   ...
   - `tv` → `159.194.208.116`
2. Для **TV** (и admin, если открываете с Xiaomi в зале) нужен режим под сеть клуба:
   - клубный Wi‑Fi **не достучался до IP Beget** → серое облако на ТВ даёт `CONNECTION_TIMED_OUT`;
   - рабочий вариант: **оранжевое** облако + в Cloudflare Network выключить **IPv6** и **HTTP/3**;
   - на ТВ открывать `http://tv.gutshotapp.ru/` (HTML без JS).  
     Подробный разбор: `docs/16-xiaomi-tv-incident-runbook.md`.
3. Проверка: с телефона в Wi‑Fi клуба `http://tv.gutshotapp.ru/` показывает блайнды.
