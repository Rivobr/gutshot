...
1. Создайте **A**-записи:
   ...
   - `tv` → `159.194.208.116`
2. Для **TV** и **admin** обязательно режим **DNS only** (серое облако).  
   Если облако оранжевое — трафик идёт через Cloudflare, и Xiaomi TV в РФ без VPN часто не открывает сайт (`ERR_CONNECTION_CLOSED` / вечная загрузка).  
   Запасной URL без VPN: `http://159.194.208.116/` (табло) и `http://159.194.208.116:8081` (админка).
3. Дождитесь распространения DNS (`dig tv.gutshotapp.ru` должен показать `159.194.208.116`, не `104.21.*`).
