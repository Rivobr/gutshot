...
1. **Табло в реальном времени**: `GET /api/v1/public/tournaments/board.html`  
   Таймер LIVE считает секунды от `levelEndsAt` (тик 250мс), данные с сервера — **каждую 1 сек**.  
   Без JS: meta refresh 1с. Внешние `.js` Xiaomi не грузит — только inline.
...
- [ ] Подождать 3–5 сек — таймер должен тикать каждую секунду (LIVE); пауза/блейнды с админки — в течение ~1с
- [ ] В закладки ТВ
